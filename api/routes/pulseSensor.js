  ...
  try {
    const result = await req.pool.query(`
      SELECT 
        ...
        (SELECT created_at FROM pulse WHERE device_id = $1 ORDER BY created_at DESC LIMIT 1) AS latest_timestamp
    `, [id]);
    ...
    console.error("❌ Error in GET /pulse/:id/latest-data", err);
    next(err);
  }
});
...
router.get("/:id/trend", verifyJWT, deviceAccessMiddleware, async (req, res, next) => {
  ...
  try {
    ...
    if (result.rows.length === 0) {
      return res.status(404).json({ message: `No pulse trend found for device '${id}'` });
    }
    ...
    console.error("❌ Error in GET /pulse/:id/trend", err);
    next(err);
  }
});

router.post("/", async (req, res, next) => {
  const { device_id, pulse } = req.body;
  try {
    const result = await req.pool.query(
      `INSERT INTO pulse (device_id, pulse)
